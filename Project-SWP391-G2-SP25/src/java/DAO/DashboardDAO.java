...
public class DashboardDAO extends DBContext {
    ...
    public int getNewlyRegisteredCustomers(java.util.Date startDate, java.util.Date endDate) {
        int count = 0;
        String sql = "SELECT COUNT(*) AS newly_registered_customers "
                + "FROM Users "
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                count = rs.getInt("newly_registered_customers");
            }
        } catch (SQLException ex) {
            Logger.getLogger(DashboardDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
